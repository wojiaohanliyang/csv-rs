...
pub unsafe fn EVP_PKEY_CTX_set1_id(
    ...
    EVP_PKEY_CTX_ctrl(ctx, -1, -1, EVP_PKEY_CTRL_SET1_ID, id_len, id as *mut c_void)
}
...
impl SM2 {
    ...
    pub fn verify(ecc_pubkey: ecc::PubKey, sig: &[u8],
        id: &Vec<u8>, msg: &Vec<u8>) -> Result<bool> {
        let mut verify_result = false;
        let pubkey_size = ecc_pubkey.g.size()?;

        unsafe {
            let eckey = EC_KEY_new_by_curve_name(NID_sm2);
            let pub_x = &ecc_pubkey.x[..pubkey_size].iter().rev().cloned().collect::<Vec<_>>();
            let pub_y = &ecc_pubkey.y[..pubkey_size].iter().rev().cloned().collect::<Vec<_>>();
            let bn_x = BN_bin2bn(
