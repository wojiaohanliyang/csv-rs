...
bitflags! {
    pub struct AttestationExtFlags: u32 {
        const EXT = 1 << 0;
        const EXT_HOST_DATA = 1 << 1;
    }
}

impl AttestationExtFlags {
    pub const EXT_U32: u32 = Self::EXT.bits();
    pub const EXT_HOST_DATA_U32: u32 = Self::EXT.bits() | Self::EXT_HOST_DATA.bits();
}

pub const CSV_HOST_DATA_SIZE: usize = 64;
...
/// this version includes additional fields: fw build, rtmr_version, rtmr[0],
/// rtmr[1], rtmr[2], rtmr[3], rtmr[4].
///
/// The V3(AttestationReportV3) introduces host_data field based on
...
impl TryFrom<&AttestationReportWrapper> for AttestationReport {
    ...
    fn try_from(report_wrapper: &AttestationReportWrapper) -> Result<Self, Self::Error> {
        match (report_wrapper.magic, report_wrapper.flags) {
            ...
            (ATTESTATION_EXT_MAGIC, AttestationExtFlags::EXT_HOST_DATA_U32) => {
...
///
/// The V3(TeeInfoV3) is tee info struct for extended attestation report.
pub enum TeeInfo<'a> {
...
impl TeeInfo<'_> {
    ...
    pub fn host_data(&self) -> &[u8] {
        match *self {
            Self::V1(_) => &[0u8; CSV_HOST_DATA_SIZE],
            Self::V2(_) => &[0u8; CSV_HOST_DATA_SIZE],
...
/// The tee_info field of AttestationReportV3.
#[repr(C)]
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub struct TeeInfoV3 {
    ...
    #[serde(with = "BigArray")]
    /// The host defined data.
    pub host_data: [u8; CSV_HOST_DATA_SIZE],
    /// The signature for the fields:
    ///   user_pubkey_digest,
    ///   vm_id,
    ///   vm_version,
...
impl Default for TeeInfoV3 {
    fn default() -> Self {
        Self {
            ...
            rtmr4: [0u8; CSV_RTMR_REG_SIZE],
            reserved1: [0u8; 592],
            host_data: [0u8; CSV_HOST_DATA_SIZE],
            sig: Default::default(),
        }
    }
}
